...
echo -e "${BLUE}🚀 FAANG-Level Traffic Generator${NC}"
echo -e "${BLUE}================================${NC}\n"
...
if curl -s -f "${GATEWAY_URL}/actuator/health" > /dev/null 2>&1 || curl -s -f "${GATEWAY_URL}/health" > /dev/null 2>&1; then
